...
from genparse.experimental.batch_inference import (
   BatchVLLM, ParallelCharacterProposal, BatchStepModel, smc, importance_sampling
)
...
def main():
    ...
    for sample in tqdm(samples):
        ...
        if args.local_poe:
            particles = importance_sampling(step_model, n_particles, return_record=args.return_record)
        else:
            particles = smc(
                step_model,
        ...
        if eval_result:
            n_correct += 1
        logger.info(f" {n_correct} out of {n_total} is correct, acc={n_correct / n_total:.3f}, "
