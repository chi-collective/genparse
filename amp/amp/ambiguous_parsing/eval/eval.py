...
def get_score_data(test_data, pred_data, test_data_lut, is_fol=False, convert=True):
    ...
    for datum_idx, (test_datum, pred_datum) in enumerate(zip(test_data, pred_data)):
        ...
        scores_by_type[ex_type]['pred_top_1_matches_either'] += (
            pred_top_1_matches_lf_0 or pred_top_1_matches_lf_1
        )
...
def get_df(test_file, eval_file, pred_path, is_fol, take_subset_by_src: bool = False):
    ...
    if take_subset_by_src:
        ...
        pred_data = [
            datum for datum in pred_data if datum['test_datum_natural'] in test_src_utts
        ]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        required=True,
        help='path to test jsonl file that was used to make predictions(no bclamp processing)',
    )
    parser.add_argument(
        '--eval_file',
        type=pathlib.Path,
        required=True,
        help='path to test eval jsonl file that has all of the interpretations',
    )
    parser.add_argument(
        '--pred_file',
        type=pathlib.Path,
    ...
    parser.add_argument(
        '--is_fol', action='store_true', help='predictions are in FOL format'
    )
