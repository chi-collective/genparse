...
class ZeroshotDatasetMetric(DatasetMetric):
        ...
        for datum_idx, (test_datum, pred_datum) in enumerate(zip(test_data, pred_data)):
            ...
            if len(top_k_outputs) < 5:
                ...
                top_k_outputs = top_k_outputs + [
                    '' for _ in range(5 - len(top_k_outputs))
                ]
    ...
    @staticmethod
    def from_bclamp_dirs(
        models_and_paths: Dict[str, List[Tuple[str]]],
        ...
        for model, per_model_data in models_and_paths.items():
            ...
            for amb_type, path in per_model_data:
                ...
                try:
                    ...
                    print(
                        f'Skipping {model} {amb_type} because len(test_data) != len(pred_data)'
                    )
                    continue

                test_data = convert_benchclamp_gold(
                    test_data, test_data_lut, is_fol=is_fol
                )
...
if __name__ == '__main__':
    ...
    for model in ['codegen-350M', 'codegen-2B', 'codegen-6B', 'codegen-16B']:
        for _type in ['scope', 'revscope', 'bound', 'conj', 'pp']:
            fol_models_and_paths[model].append(
                (
                    _type,
                    f'{model}_lamp_no_context_all_{_type}_fol_0_test_eval_constrained_bs_5_np_full',
                )
            )
    for model in ['gpt-3.5-turbo']:
        for _type in ['scope', 'revscope', 'bound', 'conj', 'pp']:
            fol_models_and_paths[model].append(
                (
                    _type,
                    f'{model}_lamp_no_context_all_{_type}_fol_0_test_eval_unconstrained-api_bs_5_np_full',
                )
            )

    metrics_by_k = {}
    for k in range(2, 6):
        big_metric_dict = ZeroshotDatasetMetric.from_bclamp_dirs(
