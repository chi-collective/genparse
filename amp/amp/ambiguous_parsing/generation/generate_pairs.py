...
def generate_pp_pairs():
    ...
    """
    visual_pp_pairs = generate_with_pp_pairs(
        vp_list=VISUAL_VPS, pp_str='with the', pp_np_list=VISUAL_INSTRUMENT_NPS
    )

    instrument_pp_pairs = generate_with_pp_pairs(
        vp_list=TACTILE_VPS, pp_str='with the', pp_np_list=TACTILE_INSTRUMENT_NPS
    )

    return visual_pp_pairs + instrument_pp_pairs


def generate_with_pp_pairs(
    vp_list=VISUAL_VPS, pp_str='with the', pp_np_list=VISUAL_INSTRUMENT_NPS
):
    """generate pairs of sentences and LFs with PP attachment ambiguities
    format of sentences:
    ...
    indef_indef_lf_template_2 = (
        'exists x . exists y . exists z . exists a . {np1}(x) AND {np2}(y) AND {np3}(z)'
    ...
    if pp_np_list == VISUAL_INSTRUMENT_NPS:
        ...
        indef_def_lf_template_1 = (
            'exists x . exists y . exists a . exists e . {np1}(x) AND {np3}(y)'
            + ' AND {vp1}(a) AND agent(a, x) AND patient(a, {np2}) AND '
            + 'have(e) AND agent(e, {np2}) AND patient(e, y)'
        )

        indef_def_lf_template_2 = (
            'exists x . exists y . exists a . {np1}(x) AND {np3}(y)'
    ...
    def_indef_lf_template_1 = (
        'exists x . exists y . exists a . exists e . {np2}(x) AND {np3}(y)'
        + ' AND {vp1}(a) AND agent(a, {np1}) AND patient(a, x) AND '
        + 'have(e) AND agent(e, x) AND patient(e, y)'
    )

    def_indef_lf_template_2 = (
        'exists x . exists y . exists a . {np2}(x) AND {np3}(y)'
    ...
    if pp_np_list == VISUAL_INSTRUMENT_NPS:
        ...
        def_def_lf_template_1 = (
            'exists x . exists a . exists e . {np3}(x)'
            + ' AND {vp1}(a) AND agent(a, {np1}) AND patient(a, {np2}) AND '
            + 'have(e) AND agent(e, {np2}) AND patient(e, x)'
        )

        def_def_lf_template_2 = (
            'exists x . exists a . {np3}(x)'
...
def generate_unambiguous_scope():
    ...
    every_intrans_template = Template(
        every_intrans_template_text, every_intrans_template_tags
    )
    ...
    each_intrans_template = Template(
        each_intrans_template_text, each_intrans_template_tags
    )
...
def generate_unambigous_bound_pronoun(is_female=True):
    ...
    def_def_subj_template = Template(
        def_def_subj_template_text, def_def_subj_template_tags
    )
    ...
    indef_def_subj_template = Template(
        indef_def_subj_template_text, indef_def_subj_template_tags
    )
    ...
    pairs += indef_def_subj_template.generate(
        indef_def_subj_lf_template, 0, 'bound_unambig'
    )

    indef_def_obj_template_text = [
        'the',
        subj_indef_choices,
        VISUAL_VPS,
        obj_name_choices,
    ...
    indef_def_obj_template = Template(
        indef_def_obj_template_text, indef_def_obj_template_tags
    )
    ...
    pairs += indef_def_obj_template.generate(
        indef_def_obj_lf_template, 0, 'bound_unambig'
    )

    def_indef_subj_template_text = [
        subj_name_choices,
        VISUAL_VPS,
        'the',
        obj_indef_choices,
    ...
    def_indef_subj_template = Template(
        def_indef_subj_template_text, def_indef_subj_template_tags
    )
    ...
    pairs += def_indef_subj_template.generate(
        def_indef_subj_lf_template, 0, 'bound_unambig'
    )

    def_indef_obj_template_text = [
        subj_name_choices,
        VISUAL_VPS,
        'the',
        obj_indef_choices,
    ...
    def_indef_obj_template = Template(
        def_indef_obj_template_text, def_indef_obj_template_tags
    )
    ...
    pairs += def_indef_obj_template.generate(
        def_indef_obj_lf_template, 0, 'bound_unambig'
    )

    indef_indef_subj_template_text = [
        'the',
        subj_indef_choices,
        VISUAL_VPS,
        'the',
    ...
    indef_indef_subj_template = Template(
        indef_indef_subj_template_text, indef_indef_subj_template_tags
    )
    ...
    pairs += indef_indef_subj_template.generate(
        indef_indef_subj_lf_template, 0, 'bound_unambig'
    )

    indef_indef_obj_template_text = [
        'the',
        subj_indef_choices,
        VISUAL_VPS,
        'the',
    ...
    indef_indef_obj_template = Template(
        indef_indef_obj_template_text, indef_indef_obj_template_tags
    )
    ...
    pairs += indef_indef_obj_template.generate(
        indef_indef_obj_lf_template, 0, 'bound_unambig'
    )
...
def generate_unambiguous_basic_helper(article='the'):
    ...
    indef_indef_transitive_template = Template(
        indef_indef_transitive_text, indef_indef_transitive_tags
    )
    indef_indef_lf_template = 'exists x . exists y . exists a . {np1}(x) AND {np2}(y) AND {vp1}(a) AND agent(a, x) AND patient(a, y)'
    pairs += indef_indef_transitive_template.generate(
        indef_indef_lf_template, 0, 'unambig'
    )

    indef_def_transitive_text = [article, INDEFINITE_SENTIENT_NPS, TRANSITIVE_VPS, NAMES]
    indef_def_transitive_tags = [None, 'np1', 'vp1', 'np2']
    indef_def_transitive_template = Template(
        indef_def_transitive_text, indef_def_transitive_tags
    )
    ...
    def_indef_transitive_template = Template(
        def_indef_transitive_text, def_indef_transitive_tags
    )
    ...
    if article == 'the':  # only generate for one article so they're not doubled
        ...
        def_def_transitive_template = Template(
            def_def_transitive_text, def_def_transitive_tags
        )
        def_def_lf_template = (
            'exists a . {vp1}(a) AND agent(a, {np1}) AND patient(a, {np2})'
        )
        pairs += def_def_transitive_template.generate(def_def_lf_template, 0, 'unambig')

    indef_intransitive_text = [
        article,
        INDEFINITE_SENTIENT_NPS,
        INTRANSITIVE_VPS + INTRANSITIVE_VPS_FOR_BOUND,
    ]
    indef_intransitive_tags = [None, 'np1', 'vp1']
    indef_intransitive_template = Template(
        indef_intransitive_text, indef_intransitive_tags
    )
    indef_intransitive_lf_template = (
        'exists x . exists a . {np1}(x) AND {vp1}(a) AND agent(a, x)'
    )
    pairs += indef_intransitive_template.generate(
        indef_intransitive_lf_template, 0, 'unambig'
    )
    ...
    if article == 'the':  # only generate for one article so they're not doubled
        ...
        pairs += def_intransitive_template.generate(
            def_intransitive_lf_template, 0, 'unambig'
        )
...
def generate_unambiguous_conj():
    ...
    conj_indef_intransitive_template = Template(
        conj_indef_intransitive_text, conj_indef_intransitive_tags
    )
    conj_indef_intransitive_lf_template = 'exists x . exists a . exists e . {np1}(x) AND {vp1}(a) AND agent(a, x) AND {vp2}(e) AND agent(e, x)'
    pairs += conj_indef_intransitive_template.generate(
        conj_indef_intransitive_lf_template, 0, 'unambig'
    )

    conj_def_intransitive_text = [
        NAMES,
        INTRANSITIVE_VPS + INTRANSITIVE_VPS_FOR_BOUND,
        'and',
        INTRANSITIVE_VPS + INTRANSITIVE_VPS_FOR_BOUND,
    ...
    conj_def_intransitive_template = Template(
        conj_def_intransitive_text, conj_def_intransitive_tags
    )
    conj_def_intransitive_lf_template = 'exists a . exists e . {vp1}(a) AND agent(a, {np1}) AND {vp2}(e) AND agent(e, {np1})'
    pairs += conj_def_intransitive_template.generate(
        conj_def_intransitive_lf_template, 0, 'unambig'
    )

    disj_indef_intransitive_text = [
        'the',
        INDEFINITE_SENTIENT_NPS,
        INTRANSITIVE_VPS + INTRANSITIVE_VPS_FOR_BOUND,
        'or',
    ...
    disj_indef_intransitive_template = Template(
        disj_indef_intransitive_text, disj_indef_intransitive_tags
    )
    disj_indef_intransitive_lf_template = 'exists x . exists a . exists e . {np1}(x) AND ( {vp1}(a) AND agent(a, x) ) OR ( {vp2}(e) AND agent(e, x) )'
    pairs += disj_indef_intransitive_template.generate(
        disj_indef_intransitive_lf_template, 0, 'unambig'
    )

    disj_def_intransitive_text = [
        NAMES,
        INTRANSITIVE_VPS + INTRANSITIVE_VPS_FOR_BOUND,
        'or',
        INTRANSITIVE_VPS + INTRANSITIVE_VPS_FOR_BOUND,
    ...
    disj_def_intransitive_template = Template(
        disj_def_intransitive_text, disj_def_intransitive_tags
    )
    disj_def_intransitive_lf_template = 'exists a . exists e . ( {vp1}(a) AND agent(a, {np1}) ) OR ( {vp2}(e) AND agent(e, {np1}) )'
    pairs += disj_def_intransitive_template.generate(
        disj_def_intransitive_lf_template, 0, 'unambig'
    )
...
def generate_unambiguous_instrument(vp_list, pp_str, pp_np_list):
    ...
    indef_indef_lf_template = (
        'exists x . exists y . exists a . {np1}(x) AND {np2}(y)'
    ...
    def_indef_lf_template = (
        'exists x . exists a . {np2}(x)'
...
def generate_unambiguous_instr_pairs():
    visual_pairs = generate_unambiguous_instrument(
        vp_list=VISUAL_VPS, pp_str='with the', pp_np_list=VISUAL_INSTRUMENT_NPS
    )

    tactile_pairs = generate_unambiguous_instrument(
        vp_list=TACTILE_VPS, pp_str='with the', pp_np_list=TACTILE_INSTRUMENT_NPS
    )
...
def generate_unambiguous_possession(np_list=VISUAL_INSTRUMENT_NPS):
    ...
    indef_lf_template = (
        'exists x . exists y . exists a . {np1}(x) AND {np2}(y) AND have(a)'
    ...
    def_lf_template = (
        'exists x . exists a . {np2}(x) AND have(a)'
...
def generate_all_pairs():
    all_pairs = {}
    all_pairs['amb_pp'] = generate_pp_pairs()
    all_pairs['unamb_pp'] = generate_unambiguous_pp()

    all_pairs['amb_scope'] = generate_scope_pairs()
    all_pairs['amb_scoperev'] = generate_reverse_scope_pairs()
    all_pairs['unamb_scope'] = generate_unambiguous_scope()

    all_pairs['amb_conj'] = generate_conjunction_pairs()
    all_pairs['unamb_conj'] = generate_unambiguous_conj()

    all_pairs['amb_boundpron'] = generate_bound_pronoun_pairs(
        is_female=True
    ) + generate_bound_pronoun_pairs(is_female=False)
    all_pairs['unamb_boundpron'] = generate_unambigous_bound_pronoun(
        is_female=True
    ) + generate_unambigous_bound_pronoun(is_female=False)

    all_pairs['unamb_basic'] = generate_unambiguous_basic()
    return all_pairs
