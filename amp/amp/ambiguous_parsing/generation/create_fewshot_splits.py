...
from ambiguous_parsing.generation.create_splits import (
...
def main(cfg: DictConfig):
    ...
    if cfg.canonicalize_train:
        (pp_pairs, unambiguous, unambig_instr, unambig_poss) = rerender_data(
            cfg, (pp_pairs, unambiguous, unambig_instr, unambig_poss)
        )
        (conj_pairs, unambig_conj, unambig_double_conj) = rerender_data(
            cfg, (conj_pairs, unambig_conj, unambig_double_conj)
        )
        (unambig_quant, scope_pairs, revscope_pairs, bound_pairs) = rerender_data(
            cfg, (unambig_quant, scope_pairs, revscope_pairs, bound_pairs)
        )
    ...
    for k, (ambig, unambig) in amb_unamb_data_by_key.items():
        (ambig_train, ambig_dev, ambig_test, unambig_train, unambig_dev, unambig_test) = (
