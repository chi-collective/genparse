...
class RandomSampler:
    ...
    def sample(self, ambiguous_data, unambiguous_data):
        ...
        ambig_index_sample = np.random.choice(
            ambig_indices, n_ambig_train + n_ambig_dev + n_ambig_test, replace=False
        )
        ...
        nonambig_index_sample = np.random.choice(
            nonambig_indices,
            n_nonambig_train + n_nonambig_dev + n_nonambig_test,
            replace=False,
        )
...
class RatioSampler:
    ...
    def get_interpretations_by_ratio(self, pairs, total_n, ratio):
        ...
        sample_ex_by_template_indices = np.random.choice(
            all_ex_by_template_indices, total_n, replace=False
        )
        sampled_ex_by_template = [
            all_ex_by_template[i] for i in sample_ex_by_template_indices
        ]
    ...
    def sample(
        ...
        for type_key in TYPE_KEYS:
            ...
            unambig_sampled_indices = np.random.choice(
                unambig_indices, n_unambig, replace=False
            )
            unambig_sampled_pairs = [
                deepcopy(unambig_pairs[i]) for i in unambig_sampled_indices
            ]
            ...
            ambig_sampled_pairs = self.get_interpretations_by_ratio(
                ambig_pairs, n_ambig, ratio
            )
...
def split_data_for_key(ambig, unambig, key, cfg):
    ...
    num_train_ambig = int(
        cfg.train.total * cfg.train[f'perc_{key}_ambig'] * cfg.train[f'perc_{key}']
    )
    num_dev_ambig = int(
        cfg.dev.total * cfg.dev[f'perc_{key}_ambig'] * cfg.dev[f'perc_{key}']
    )
    num_test_ambig = int(
        cfg.test.total * cfg.test[f'perc_{key}_ambig'] * cfg.test[f'perc_{key}']
    )
    num_train_unambig = int(
        cfg.train.total * (1 - cfg.train[f'perc_{key}_ambig']) * cfg.train[f'perc_{key}']
    )
    num_dev_unambig = int(
        cfg.dev.total * (1 - cfg.dev[f'perc_{key}_ambig']) * cfg.dev[f'perc_{key}']
    )
    num_test_unambig = int(
        cfg.test.total * (1 - cfg.test[f'perc_{key}_ambig']) * cfg.test[f'perc_{key}']
    )

    ambig_train, ambig_dev, ambig_test = split_keep_paired(
        ambig, num_train_ambig, num_dev_ambig, num_test_ambig
    )

    unambig_train, unambig_dev, unambig_test = split_keep_paired(
        unambig, num_train_unambig, num_dev_unambig, num_test_unambig
    )
...
def check_and_fill_config(cfg):
    for split in ['train', 'dev', 'test']:
        valid_type_keys = set(
            [
                '_'.join(k.split('_')[1:])
                for k in cfg[split].keys()
                if k.startswith('perc_')
            ]
        )
        ...
        if any([perc is None for perc in percs]):
            if not all([perc is None for perc in percs]):
                raise ValueError(
                    'If you specify any of the perc_*_ambig or perc_*_unambig, you must specify all of them.'
                )
...
def main(cfg: DictConfig):
    ...
    if cfg.sampler == 'random':
        ambiguous_data = (
            pp_pairs + scope_pairs + reverse_scope_pairs + conj_pairs + bound_pairs
        )
        unambiguous_data = (
            unambig_pp + unambig_scope + unambig_conj + unambig_bound + unambiguous
        )
        ...
        amb_unamb_data_by_key = {
            'pp': (pp_pairs, unambig_pp),
        ...
        for k, (ambig, unambig) in amb_unamb_data_by_key.items():
            (
                ...
                unambig_dev,
                unambig_test,
            ) = split_data_for_key(ambig, unambig, k, cfg)
            ...
            ambig_nums_per_type['train'][k] = int(
                cfg.train[f'perc_{k}'] * cfg.train[f'perc_{k}_ambig'] * cfg.train.total
            )
            ambig_nums_per_type['dev'][k] = int(
                cfg.dev[f'perc_{k}'] * cfg.dev[f'perc_{k}_ambig'] * cfg.dev.total
            )
            ambig_nums_per_type['test'][k] = int(
                cfg.test[f'perc_{k}'] * cfg.test[f'perc_{k}_ambig'] * cfg.test.total
            )

            unambig_nums_per_type['train'][k] = int(
                cfg.train[f'perc_{k}'] * cfg.train[f'perc_{k}_unambig'] * cfg.train.total
            )
            unambig_nums_per_type['dev'][k] = int(
                cfg.dev[f'perc_{k}'] * cfg.dev[f'perc_{k}_unambig'] * cfg.dev.total
            )
            unambig_nums_per_type['test'][k] = int(
                cfg.test[f'perc_{k}'] * cfg.test[f'perc_{k}_unambig'] * cfg.test.total
            )

        unambiguous_train, unambiguous_dev, unambiguous_test = split_random(
            unambiguous,
        ...
        unambig_nums_per_type['train']['unambiguous'] = int(
            cfg.train.perc_unambig * cfg.train.total
        )
        unambig_nums_per_type['dev']['unambiguous'] = int(
            cfg.dev.perc_unambig * cfg.dev.total
        )
        unambig_nums_per_type['test']['unambiguous'] = int(
            cfg.test.perc_unambig * cfg.test.total
        )
