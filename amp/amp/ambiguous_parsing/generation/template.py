...
class Template:
    def __init__(self, template_list, template_tags):
        ...
        for i, element in enumerate(self.template_list):
            if not isinstance(element, list):
                ...
                if isinstance(self.template_tags[i], str):
                    raise Exception(
                        'template tag must be None if template element is not a list'
                    )
    ...
    def generate(self, lf_template, template_idx, amb_type):
        ...
        for surface in all_surface:
            ...
            product_of_options = [
                x for x in itertools.product(*[x[1] for x in var_bindings])
            ]
