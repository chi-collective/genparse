...
def test_connection_palindrome():
    ...
    print('basic tree sum:', cfg.agenda())
    ...
    other = lambda x: f'~{x}'
    free = lambda x: f'free({x})'
    new = cfg.prefix_grammar

    meow = new._parse_chart(s)

    print(dict(keep_nonzero(Real, meow)))

    D = cfg.derivatives(s)[-1]
    ...
    derp = Real.chart()
    for k, v in D.agenda().items():
        derp[str(k)] += v

    print(dict(keep_nonzero(Real, derp)))
    ...
    for key in meow:
        ...
        if K - I == 0: continue
        if not str(X).startswith('~'): continue
        m = meow[key]

        slashes = '/'.join(s[I:K])

        slash_key = f'{X[1:]}/{slashes}'
        d = derp[slash_key]
        ...
        if 0:
            ...
            print(d)
            print(slash_key)
            print(colors.mark(m.metric(d) <= 1e-5))

        all_equal &= m.metric(d) <= 1e-5
...
def assert_equal(have, want, tol=1e-8):
    ...
    if THROW:
        ...
        if error <= tol:
            print(colors.mark(error <= tol), f'have = {have}, want = {want}, error = {error}')
        else:
            print(colors.mark(error <= tol), f'have = {have}, want = {want}, error = {error}')
