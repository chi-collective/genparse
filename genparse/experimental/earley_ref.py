...
class Earley:
    ...
    def __call__(self, x):
        ...
        last_col = self.chart(x)

        value = last_col.c_chart.get((self._initial_column, self.cfg.S))
    ...
    def _compute_chart(self, x):
        if len(x) == 0:
            ...
            prev_col = self.chart(x[:-1])
            return self.next_column(prev_col, x[-1])

    def p_next(self, prefix):
        return self.next_token_weights(self.chart(prefix))

    def next_column(self, prev_col, token):
        next_col = Column(prev_col.k + 1)
    ...
    def _update(self, col, I, X, Ys, value):
        if Ys == 0:
            ...
            if was is None:
                col.Q[item] = -((col.k - I.k) * self.ORDER_MAX + self.order[X])
    ...
    #
    def next_token_weights(self, col):
        is_terminal = self.cfg.is_terminal
        zero = self.cfg.R.zero

        q = {}
        q[self._initial_column, self.cfg.S] = self.cfg.R.one
        ...
        for Y in col_waiting_for:
            if is_terminal(Y):
                ...
                for I, X, Ys in col_waiting_for[Y]:
                    if self.unit_Ys[Ys]:
                        node = (I, X)
                        value = self._helper(node, q)
                        total += col_i_chart[I, X, Ys] * value
                p[Y] = total

        return p

    def _helper(self, top, q):
        value = q.get(top)
        ...
        while stack:
            ...
            if node.edges is None:
                node.edges = [x for x in J.waiting_for.get(Y, ()) if self.unit_Ys[x[2]]]
            ...
            # cursor is at the end, all neighbors are done
                ...
                if neighbor_value is None:
                    ...
                    node.cursor += 1
                    node.value += J.i_chart[arc] * neighbor_value
